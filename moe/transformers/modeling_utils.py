...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def _load_state_dict_into_model(cls, model, state_dict, pretrained_model_name_or_path, _fast_init=True):
        ...
        if hasattr(model.config, 'use_moe') and model.config.use_moe == 'MoE':
            load_MoE(model, state_dict, loaded_keys, missing_keys, unexpected_keys, model.config.model_type)

        if hasattr(model.config, 'use_moe') and model.config.use_moe == 'SMoE':
            load_SMoE(model, state_dict, loaded_keys, missing_keys, unexpected_keys, model.config.model_type)
        ...
        for key in loaded_keys:  # todo load experts
            if moe_note in key and 'attention' not in key:
                ...
                if key in unexpected_keys:
                    unexpected_keys.remove(key)
                for layer in range(model.config.n_experts):
                    moe_key = key.replace('.ffn_output.',
                                          '.ffn_output.experts.' + str(layer) + '.dense.')  # used in albert
                    state_dict[moe_key] = values
                    # todo remove moe_key in single model
                    if moe_key in missing_keys:
                        missing_keys.remove(moe_key)
...
def load_SMoE(model, state_dict, loaded_keys, missing_keys, unexpected_keys, model_type):
    loaded_keys.sort()
    missing_keys.sort()
    unexpected_keys.sort()
    if model_type == 'bert' or model_type == 'roberta' or model_type == 'electra':
        moe_notes = ['output.dense', 'intermediate.dense']
        for key in loaded_keys:  # todo load experts
