...
class SynFlow(Pruner):
    ...
    def score(self, model, dataloader, device, compute_loss=None):
        ...
        latent_dim = model.module.pretrained_model.roberta.embeddings.position_embeddings.embedding_dim # roberta model
        inputs = next(iter(dataloader))
        input_ids = inputs['input_ids']
        output = 1e-7 * torch.ones([1] + list(input_ids[0, :].shape) + [latent_dim]).to(device)
        num = 1
        with autocast():
            inputs = next(iter(dataloader))
            output = model(inputs, synflow=True)
            torch.sum(output)
            loss = torch.mean(output)
        loss.backward()
        for _, p in self.masked_parameters:
            self.scores[id(p)] = torch.clone(p.grad * p).detach().abs_()
            self.scores[id(p)] = torch.where(torch.isinf(self.scores[id(p)]), torch.full_like(self.scores[id(p)], 1), self.scores[id(p)])
            p.grad.data.zero_()
