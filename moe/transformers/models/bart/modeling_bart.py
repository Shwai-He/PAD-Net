...
class BartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig, layer_id=None):
        ...
        if hasattr(config, 'use_moe'):
            if config.use_moe == 'MoE':
                self.fc1 = MoE(self.embed_dim, config.decoder_ffn_dim)
            else:
                self.fc1 = SMoE(self.embed_dim, config.decoder_ffn_dim)
        else:
            self.fc1 = nn.Linear(self.embed_dim, config.decoder_ffn_dim)
        if hasattr(config, 'use_moe'):
            if config.use_moe == 'MoE':
                self.fc2 = MoE(config.decoder_ffn_dim, self.embed_dim)
            else:
                self.fc2 = SMoE(config.decoder_ffn_dim, self.embed_dim)
        else:
