...
from transformers.models.layers import MoE, SMoE, AMoE, STRMoE, PadMoE


MoE_Dict = {'MoE': MoE, 'SMoE': SMoE, 'AMoE': AMoE, 'STRMoE': STRMoE, 'PadMoE': PadMoE}
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if self.config.pos_moe in ['int', 'both']:
            if hasattr(config, 'use_moe') and config.use_moe in MoE_Dict:
                if config.use_moe == 'AMoE':
                    self.dense = AMoE(config.hidden_size, config.intermediate_size, config, activation=self.intermediate_act_fn)
                else:
    ...
    def forward(self, hidden_states):
        moe_loss = None
        if isinstance(self.dense, (MoE, AMoE, SMoE, STRMoE, PadMoE)):
            hidden_states, moe_loss = self.dense(hidden_states)
        else:
            hidden_states = self.dense(hidden_states)
        if not isinstance(self.dense, AMoE):
...
class BertOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        if self.config.pos_moe in ['out', 'both']:
            if hasattr(config, 'use_moe') and config.use_moe in MoE_Dict:
...
class BertLayer(nn.Module):
    ...
    def feed_forward_chunk(self, attention_output):
        intermediate_output, moe_loss_inter = self.intermediate(attention_output)
        layer_output, moe_loss_out = self.output(intermediate_output, attention_output)
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        moe_loss = 0 if (hasattr(self.config, 'use_moe') and self.config.use_moe in ['MoE', 'SMoE', 'AMoE', 'PadMoE']) else None
...
class SMoE(nn.Module):
    ...
    def __init__(self, intermediate_size, config, noisy_gating=True, device='cpu'):
        ...
        self.weight = nn.Parameter(torch.Tensor(
            self.config.n_experts, self.output_size, self.hidden_size), requires_grad=True)
        self.bias = nn.Parameter(torch.Tensor(
            self.config.n_experts, self.output_size), requires_grad=True)
        self.w_gate = nn.Parameter(torch.zeros(intermediate_size, config.n_experts), requires_grad=True).to(self.device)
        self.w_noise = nn.Parameter(torch.zeros(intermediate_size, config.n_experts), requires_grad=True).to(self.device)
        self.mean = nn.Parameter(torch.tensor([0.0]), requires_grad=False).to(self.device)
        self.std = nn.Parameter(torch.tensor([1.0]), requires_grad=False).to(self.device)
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        # calculate importance loss
        importance = gates.view(-1, self.num_experts).sum(0)
        loss = self.cv_squared(importance) + self.cv_squared(load)
        loss *= loss_coef
