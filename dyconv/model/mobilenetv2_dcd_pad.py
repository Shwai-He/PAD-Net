...
class DYCls(nn.Module):
    def __init__(self, inp, oup):
        ...
        self.fc_phi_static = nn.Linear(mid, self.dim ** 2, bias=False)
        self.fc_scale_static = nn.Linear(mid, oup, bias=False)
        self.hs = Hsigmoid()
    ...
    def forward(self, x):
        ...
        dy_phi = (F.linear(y, self.phi * self.phi_mask + self.phi_static * (1 - self.phi_mask), None)).view(b, self.dim, self.dim)
        # dy_scale = self.hs(self.fc_scale(y)).view(b, -1)
        dy_scale = self.hs(F.linear(y, self.scale * self.scale_mask + self.scale_static * (1 - self.scale_mask), None)).view(b, -1)
...
class DYModule(nn.Module):
    def __init__(self, inp, oup, fc_squeeze=8):
        ...
        self.fc_scale_static = nn.Linear(squeeze, oup, bias=False)
        self.hs = Hsigmoid()
        self.phi = self.fc_phi.weight
        self.phi_static = self.fc_phi_static.weight
        self.scale = self.fc_scale.weight
    ...
    def forward(self, x):
        ...
        dy_scale = self.hs(F.linear(y, self.scale * self.scale_mask + self.scale_static * (1 - self.scale_mask), None)).view(b,-1,1,1)
...
class MobileNetV2_dcd(nn.Module):
    ...
    def reset_buffers(self):
        for name, module in self.named_modules():
            if isinstance(module, (DYCls, DYModule)):
                mask = module.phi_mask
                print(mask.numel(), mask.mean())
                module.register_buffer('phi_mask', mask)
                mask = module.scale_mask
                print(mask.numel(), mask.mean())
                module.register_buffer('scale_mask', mask)

def mobilenetv2_dcd_cmd(**kwargs):
    """
