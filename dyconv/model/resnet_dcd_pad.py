...
__all__ = ['ResNet_dcd_cmd', 'resnet18_dcd_cmd', 'resnet34_dcd_cmd', 'resnet50_dcd_cmd',
           'resnet101_dcd_cmd', 'resnet152_dcd_cmd']
...
class conv_basic_dy(nn.Module):
    def __init__(self, inplanes, planes, stride):
        ...
        self.fc_phi_static = nn.Linear(squeeze, self.dim ** 2, bias=False)
        self.fc_scale_static = nn.Linear(squeeze, planes, bias=False)
        self.hs = Hsigmoid()
        self.phi = self.fc_phi.weight
        self.phi_static = self.fc_phi_static.weight
        self.scale = self.fc_scale.weight
        self.scale_static = self.fc_scale_static.weight
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                if m.bias:
                    m.bias.data.zero_()
        self.register_buffer('phi_mask', torch.ones(self.fc_phi.weight.shape[0], 1))
        self.register_buffer('scale_mask', torch.ones(self.fc_scale.weight.shape[0], 1))
        del self.fc_scale, self.fc_phi, self.fc_phi_static, self.fc_scale_static
    ...
    def forward(self, x):
        ...
        phi = (F.linear(y, self.phi * self.phi_mask + self.phi_static * (1 - self.phi_mask), None)).view(b, self.dim, self.dim)
        # phi_ = (F.linear(y, self.phi, None)).view(b, self.dim, self.dim)
        # scale = self.hs(self.fc_scale(y)).view(b,-1,1,1)
        scale = self.hs(F.linear(y, self.scale * self.scale_mask + self.scale_static * (1 - self.scale_mask), None)).view(b,-1,1,1)
        # scale_ = self.hs(F.linear(y, self.scale, None)).view(b,-1,1,1)
...
class ResNet_dcd_cmd(nn.Module):

    def __init__(self, block, layers, num_classes=1000, dropout=None):
        self.inplanes = 64
        super(ResNet_dcd_cmd, self).__init__()
    ...
    def reset_buffers(self):
        ...
        for name, module in self.named_modules():
            if isinstance(module, (conv_dy, conv_basic_dy)):
...
def resnet10_dcd_pad(pretrained=False, **kwargs):
    """Constructs a ResNet-10 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_dcd_cmd(BasicBlock_dy, [1, 1, 1, 1], **kwargs)
    # if pretrained:
    #     model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model

def resnet18_dcd_pad(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_dcd_cmd(BasicBlock_dy, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34_dcd_pad(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_dcd_cmd(BasicBlock_dy, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50_dcd_pad(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_dcd_cmd(Bottleneck_dy, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet']))
    return model


def resnet101_dcd_pad(pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_dcd_cmd(Bottleneck_dy, [3, 4, 23, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet101']))
    return model


def resnet152_dcd_pad(pretrained=False, **kwargs):
    """Constructs a ResNet-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_dcd_cmd(Bottleneck_dy, [3, 8, 36, 3], **kwargs)
    if pretrained:
